...
- 项目条目使用“简介、技术特点、适用场景与价值、限制与来源”。简介先交代它是什么、给谁用、解决什么问题；“进入日榜”只放在来源标签中。真正的发布、公告或安全事件则用“事件简介”交代变化与背景。
- 技术特点要讲清具体设计、机制和它解决的难题，不能只列通用功能或堆砌“统一运行时”等词语；没有证据时不声称首创或优于其他方案。
- 价值要结合具体使用场景，说明谁原来需要做什么、这个项目如何改变流程、因此省下什么工作。不能只写“工程规范版本化”或“降低迁移成本”等没有背景和因果解释的结论。
- 每条正文以约 250～450 个汉字为参考，按需用两三句连贯的话讲清一个部分；短句不能省略必要背景，证据不足也不能靠空话凑长度。“值得继续观察”同样需要完整简介和具体机制。
- 写作前核实当前仓库的准确身份，读取 README 中的介绍、缘起、技术设计、使用方式和限制；抓取成功或得到页面标题不等于读懂了来源。不能用旧知识、同名项目或往期报告代替当前原文，关键信息无法读到时应舍弃候选。
...
最终检查时应逐条对照来源，再把报告通读一遍：读者能否说清“它是什么、具体怎么做、什么情况下有用”？如果项目身份不符、论断没有来源支持、句子像英文直译，或只有缺少背景的一句话结论，就重新核验并改写。实际执行规则以 `.github/prompts/daily-tech-trends.md` 为准。
